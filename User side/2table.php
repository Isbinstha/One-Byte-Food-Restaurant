...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt_insert = $conn->prepare("INSERT INTO booked_tables_two(table_number, booking_date, booking_time, user_name, user_email) VALUES (?, ?, ?, ?, ?)");
    foreach ($selected_tables as $table_id) {
...
<body>
    ...
    function updateFormFields() {
        ...
        var totalAmount = numTables * 200; // Assuming each table costs $100
    ...
    function reserveTable() {
        ...
        var params = 'date=' + encodeURIComponent(date) + '&time=' + encodeURIComponent(time) + '&seats=' + encodeURIComponent(selectedTables.length * 2) + '&cost=' + encodeURIComponent(selectedTables.length * 200) + '&selected_tables=' + encodeURIComponent(JSON.stringify(selectedTables)) + '&user_name=' + encodeURIComponent(userName) + '&user_email=' + encodeURIComponent(userEmail);
        xhr.send(params);
    }
