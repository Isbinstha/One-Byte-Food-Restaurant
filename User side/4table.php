...
$sql_availability = "SELECT TableID, TableName, Availability FROM tables_four"; // Update table name to tables
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($selected_tables as $table_id) {
        $sql_update_availability = "UPDATE tables_four SET Availability = 'reserved' WHERE TableID = ?";
        $stmt = $conn->prepare($sql_update_availability);
        $stmt->bind_param("i", $table_id);
        $stmt->execute();
    }

    // Insert reservation details into the booked_tables table
    $stmt_insert = $conn->prepare("INSERT INTO booked_tables_four(table_number, booking_date, booking_time, user_name, user_email) VALUES (?, ?, ?, ?, ?)");
    foreach ($selected_tables as $table_id) {
...
<head>
    ...
    <link rel="stylesheet" href="4table.css">
</head>
<body>
    ...
    function updateFormFields() {
        ...
        document.getElementById('seats').value = numTables * 4; // Assuming each table has 2 seats

        // Update the total amount displayed
        var totalAmount = numTables * 400; // Assuming each table costs $100
    ...
    var params = 'date=' + encodeURIComponent(date) + '&time=' + encodeURIComponent(time) + '&seats=' + encodeURIComponent(selectedTables.length * 4) + '&cost=' + encodeURIComponent(selectedTables.length * 400) + '&selected_tables=' + encodeURIComponent(JSON.stringify(selectedTables)) + '&user_name=' + encodeURIComponent(userName) + '&user_email=' + encodeURIComponent(userEmail);
    xhr.send(params);
}
    ...
    if (!<?php echo json_encode($loggedIn); ?>) {
        ...
        window.location.href = 'payment_four.php?amount=' + totalCost + '&tables=' + tablesParam + '&date=' + date + '&time=' + time + '&user_name=' + userName + '&user_email=' + userEmail;
    }
